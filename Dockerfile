...
RUN CGO_ENABLED=0 GOOS=$TARGETOS GOARCH=$TARGETARCH make -C /src install PREFIX=/pkg GOTOOLCHAIN=local
...
RUN set -x \
  && exporter --version 2>/dev/null \
  && keepalive-check --version 2>/dev/null

ARG BININFO_BUILD_DATE BININFO_COMMIT_HASH BININFO_VERSION
LABEL source_repository="https://github.com/sapcc/http-keep-alive-monitor" \
  org.opencontainers.image.url="https://github.com/sapcc/http-keep-alive-monitor" \
